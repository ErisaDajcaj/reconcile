...
### Pipeline
1. **Deterministic pass** *(pure code, no LLM).* Exact join on **`(order-id/reference, amount, currency)`**. Clears the easy 80–95%. Emits confirmed matches directly (still audited); passes only the **residual** downstream. *Design decision (Plan 1): the key includes `currency` — so `10 USD` never matches `10 EUR` — and deliberately **excludes date**, because settlement-date skew is a fuzzy signal assigned to the matcher agent (see step 3), not a reason to reject an otherwise-exact deterministic match.*
